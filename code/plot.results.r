...
#      par(oma = c(1,1,1,1), mar = c(4,4,0,0), mfrow = c(1,1))
      ...
      axis(1, lwd = 2, cex.axis = 1.5)
      axis(2, lwd = 2, cex.axis = 1.5)
      for(i in day.sta.ind) lines(plen, exp(y$station_pred_eta[i,ind[,1]]), col = dcoli, lwd = 1)
      lines(plen, exp(z[ind[,1],1]), lwd = 2, col = "dodgerblue")
      polygon(c(plen,rev(plen)), exp(c(z[ind[,1],2],rev(z[ind[,1],3]))), col = dcol, border = "transparent")
      ...
      tdat = tdat[which(tdat[,2] > 0),]
      points(jitter(tdat[,1], factor = 2), tdat[,2], pch = 19, col = dcoli)
    #  mtext(side = 2, ylab, line = 2, cex = 1.5)
      #mtext(side = 3, "Day", line = 1, cex = 1.5)
      if(boot) lines(plen, exp(apply(boot.pred.eta[,ind[,1]], 2, quantile, probs = 0.025, na.rm = TRUE)), col = 'dodgerblue', lty = 2, lwd = 2)
      if(boot) lines(plen, exp(apply(boot.pred.eta[,ind[,1]], 2, quantile, probs = 0.975, na.rm = TRUE)), col = 'dodgerblue', lty = 2, lwd = 2)
      
      #plot(plen, exp(z[ind[,2],1]), type = 'n', ylim = c(0, ymax[length(ymax)]), xlim = xlim, axes = FALSE, ann = FALSE)
      #box(lwd = 2)
      #grid(col = gray(0.7), lwd = 1, lty = 2)
      for(i in night.sta.ind) lines(plen, exp(y$station_pred_eta[i,ind[,2]]), col = ncoli, lwd = 1)
      lines(plen, exp(z[ind[,2],1]), lwd = 2, col = "orangered")
      polygon(c(plen,rev(plen)), exp(c(z[ind[,2],2],rev(z[ind[,2],3]))), col = ncol, border = "transparent")
      #abline(h = 1, col = "red", lwd =2)
      #rug(jitter(x$day$cl.less$length, factor = 2), ticksize = 0.03)
      tdat = cbind(dat$length, (exp(-dat$offst) * dat$recnumlen.ch/dat$recnumlen.rh))[dat$dn == "night",]
      tdat = tdat[which(tdat[,2] > 0),]
      points(jitter(tdat[,1], factor = 2), tdat[,2], pch = 19, col=ncoli)
      #mtext(side = 3, "Night", line = 1, cex = 1.5)
#      mtext(side = 1, 'Length (cm)', line = -1, cex = 1.5, outer = TRUE)
      if(boot) lines(plen, exp(apply(boot.pred.eta[,ind[,2]], 2, quantile, probs = 0.025, na.rm = TRUE)), col = 'orangered', lty = 2, lwd = 2)
      if(boot) lines(plen, exp(apply(boot.pred.eta[,ind[,2]], 2, quantile, probs = 0.975, na.rm = TRUE)), col = 'orangered', lty = 2, lwd = 2)
    #dev.off()
  }  
#  setwd(parentdir)
