  ...
  years, pred.length.integer = NULL, n.boot = 1000, do.spring = TRUE, do.fall = TRUE, summ.file.mod = "", min.length.calibrate, max.length.calibrate)
{
  ...
  if(is.null(pred.length.integer)) pred.length.integer = as.integer(read.csv(paste0("results/", stock,".spring.Nal.csv"))[[1]])
  ...
  print(3)
    colnames(boot.ratio.res) = colnames(boot.res) = c("cv.fall", "lo.fall", "hi.fall", "cv.spring", "lo.spring", "hi.spring")
