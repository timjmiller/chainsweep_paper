  ...
  years, pred.length.integer = NULL, n.boot = 1000, do.spring = TRUE, do.fall = TRUE, summ.file.mod = "", min.length.calibrate, max.length.calibrate)
{
  ...
  if(NCOL(boot.pred.eta)> length(twintrawl.dn.res$pred.length)) #day first, night second
  {
    small.boot.eta.night = boot.pred.eta[,length(twintrawl.dn.res$pred.length) + pred.eta.index.for.survey]
  }
  ...
  if(do.boot.biomass)
  {
    for(y in years)
    {
      ...
      fn = paste0(stock, "_boot_", trac.year)
  ...
  if(do.boot.res)
  {
    ...
    if(do.spring | do.fall) {
      ...
      write.csv(round(boot.res,2), file = paste0(summ.file.mod, stock, ".cv.biomass.csv"))
      write.csv(round(boot.ratio.res,2), file = paste0(summ.file.mod, stock, ".cv.biomass.ratio.csv"))
    }
  }
