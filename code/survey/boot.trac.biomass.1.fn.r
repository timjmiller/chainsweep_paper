boot.trac.biomass.1.fn = function(fall.lendat = all.fall.lendat[[7]], spring.lendat = all.spring.lendat[[8]], dfo.lendat = all.dfo.lendat[[7]],
  fall.lw.dat, spring.lw.dat, dfo.lw.dat, boot.pred.eta, nefsc.str.size, dfo.str.size, include.fall=TRUE, include.spring = TRUE, include.dfo = TRUE, fit.models = TRUE, lengths = 1:50)
{
  ...
  if(include.fall)
  {
    ...
    if(fit.models)
    {
      ...
      for(i in lengths) 
      {
        ind = which(calib.boot.fall.lendat$LENGTH == i)
        calib.boot.fall.lendat$EXPNUMLEN[ind] = exp(boot.pred.eta[which(lengths== i)])*calib.boot.fall.lendat$EXPNUMLEN[ind] #change rockhopper night to chainsweep night
      }
  ...
  if(include.spring)
  {
    ...
    if(fit.models)
    {
      ...
      for(i in lengths) 
      {
        ind = which(calib.boot.spring.lendat$LENGTH == i)
        calib.boot.spring.lendat$EXPNUMLEN[ind] = exp(boot.pred.eta[which(lengths== i)])*calib.boot.spring.lendat$EXPNUMLEN[ind] #change rockhopper night to chainsweep night
      }
  ...
  if(include.dfo)
  {
    ...
    if(fit.models)
    {
      ...
      for(i in lengths) 
      {
        ...
        if(length(ind)) calib.boot.dfo.lendat[,ind] = exp(boot.pred.eta[which(lengths== i)])*calib.boot.dfo.lendat[,ind] #change rockhopper night to chainsweep night
      }
