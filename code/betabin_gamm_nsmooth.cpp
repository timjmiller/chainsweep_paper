...
Type objective_function<Type>::operator() ()
{
  ...
  DATA_MATRIX(phi_X); //nobs x n_phi_betas
  DATA_VECTOR(phi_Dplus_diag); //n_phi_smooth_reff
  DATA_IVECTOR(phi_beta_ns_ind); //which of the full beta vector are fixed effects.
  DATA_IVECTOR(phi_fp_mean_smooth_re); //(nsmu) first column of X for each smoothers random effects
  DATA_IVECTOR(phi_lp_mean_smooth_re); //(nsmu) last column of X for each smoothers random effects
  DATA_INTEGER(use_phi_mean_smooth_re);
  ...
  PARAMETER_VECTOR(phi_beta); //n_phi_beta //fixed effects for log of beta-binomial dispersion parameter
  PARAMETER_VECTOR(phi_mean_smooth_re); //n_phi_smooth_reff // population-level average random effects portion of smoother
  PARAMETER_VECTOR(phi_lambda_par); //log smoothing parameters for beta-binomial dispersion parameter
  ...
  vector<Type> nll_phi_mean_smooth(phi_mean_smooth_re.size());
  nll_phi_mean_smooth.fill(0);
  for(int i = 0; i < phi_beta_ns_ind.size(); i++) phi_beta_use(phi_beta_ns_ind(i)) = phi_beta(i);
	if(use_phi_mean_smooth_re == 1) 
  {
    int pp = 0;
	  if(phi_nsmu > 0) for(int s = 0; s < phi_nsmu; s++) for(int i = phi_fp_mean_smooth_re(s); i <= phi_lp_mean_smooth_re(s); i++)//for(int j = 0; j < mean_smooth_re.size(); j++) 
	  {
	    nll_phi_mean_smooth(pp) -= dnorm(phi_mean_smooth_re(pp), Type(0.0), exp(-0.5*(log(phi_Dplus_diag(pp)) + phi_lambda_par(s))), 1);
	    SIMULATE phi_mean_smooth_re(pp) = rnorm(Type(0.0), exp(-0.5*(log(phi_Dplus_diag(pp)) + phi_lambda_par(s))));
      phi_beta_use(i) = phi_mean_smooth_re(pp);
      pp++;
	  }
    nll += nll_phi_mean_smooth.sum();
    REPORT(nll_phi_mean_smooth);
	}
  
  vector<Type> eta = offst + X * beta_use;
  vector<Type> phi_eta = phi_X * phi_beta_use;
  vector<Type> phi = exp(phi_eta);
  ...
  for(int j = 0; j < N; j++) 
  {
    ...
    nll_obs(j) -= dbetabinom(y(j),n(j),mu(j),phi(j),1);
    SIMULATE y(j) = rbetabinom(n(j),mu(j),phi(j));
  }
  nll += nll_obs.sum();
  REPORT(nll_obs);
  SIMULATE 
  {
    ...
    ADREPORT(beta_re_use);
    ADREPORT(beta_use);
    ADREPORT(phi_beta_use);
  }
	return(nll);
}
