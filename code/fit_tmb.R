...
{
  model$opt <- try(stats::nlminb(model$par, model$fn, model$gr, control = list(iter.max = 1000, eval.max = 1000)))
  
  if(!is.character(model$opt) & n.newton){ # Take a few extra newton steps
  ...
  #model$parList = model$env$parList(x=model$env$last.par.best)
