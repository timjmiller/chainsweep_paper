bootstrap.cl.less = function(cl.less, by.day.night=TRUE)
{
  
  if(by.day.night) #bootstrap is conditioned on the number of day and night tows.
  {
  ...
  else #bootstrap is not conditioned on the number of day and night tows.
  {
    ids = unique(cl.less$id)
    ids.boot = sample(ids, replace = TRUE)
    n.per.sta.boot = sapply(ids.boot, function(x) sum(cl.less$id == x))
    id.boot = rep(1:length(ids.boot), n.per.sta.boot)
    index.boot = unlist(sapply(ids.boot, function(x) which(cl.less$id == x)))
    cl.less = cl.less[index.boot,]
    cl.less$id = id.boot
