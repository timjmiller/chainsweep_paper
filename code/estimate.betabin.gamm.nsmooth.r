estimate.betabin.gamm.nsmooth <- function(
  dat,
...
)
{
  ...
  if(missing(input))
  {
    ...
    temp.dat = gam(phi.mean.form, family = binomial, data = dat, fit = FALSE, control = list(scalePenalty=FALSE))  
    temp.fit = gam(phi.mean.form, family = binomial, data = dat, control = list(scalePenalty=FALSE))  
    pred.dat = predict(temp.fit, newdata = new_dat, type = "lpmatrix")
    
    phi.dat <- list(X = temp.dat$X)
    phi.dat$Dplus_diag = numeric()
    phi.dat$fp_mean_smooth_re = integer()
    phi.dat$lp_mean_smooth_re = integer()
    phi.dat$Dplus_diag = numeric()  
    phi.dat$Xpred = pred.dat
    not_phi_mean_smooth_re_ind = 1:NCOL(phi.dat$X)
    phi_mean_smooth_re_ind = integer()
    ...
    if(length(temp.dat$smooth))
    {
      for(i in 1:length(temp.dat$smooth))
      {
        S <- temp.dat$smooth[[i]]$S[[1]]
        UDU <- eigen(S)
        phi.dat$Dplus_diag <- c(phi.dat$Dplus_diag, UDU$val[which(UDU$val > 1e-10)])
        UF <- cbind(UDU$vec[,which(UDU$val <= 1e-10)])
        UR <- cbind(UDU$vec[,which(UDU$val > 1e-10)])
        ind = temp.dat$smooth[[i]]$first.para:temp.dat$smooth[[i]]$last.para
        phi.dat$X[,ind] = cbind(cbind(phi.dat$X[,ind]) %*% UF, cbind(phi.dat$X[,ind]) %*% UR)
        phi.dat$Xpred[,ind] = cbind(cbind(phi.dat$Xpred[,ind]) %*% UF, cbind(phi.dat$Xpred[,ind]) %*% UR)
        ind = ind[-(1:NCOL(UF))]
        phi.dat$fp_mean_smooth_re = c(phi.dat$fp_mean_smooth_re, min(ind))
        phi.dat$lp_mean_smooth_re = c(phi.dat$lp_mean_smooth_re, max(ind))
        phi_mean_smooth_re_ind = c(phi_mean_smooth_re_ind, ind)
      }
      not_phi_mean_smooth_re_ind = not_phi_mean_smooth_re_ind[-phi_mean_smooth_re_ind]
    }
    ...
    dat = list(
      ...
      Zpred = mu.dat$Zpred,
      phi_X = phi.dat$X,
      phi_Dplus_diag = phi.dat$Dplus_diag,
      phi_beta_ns_ind = not_phi_mean_smooth_re_ind - 1, #because of C++
      phi_fp_mean_smooth_re = phi.dat$fp_mean_smooth_re - 1, #because of C++
      phi_lp_mean_smooth_re = phi.dat$lp_mean_smooth_re - 1, #because of C++
      use_phi_mean_smooth_re = use_phi_mean_smooth_re
    )
    ...
    par = list(
      ...
      phi_beta = rep(0, length(not_phi_mean_smooth_re_ind)),
      phi_mean_smooth_re = rep(0, length(phi_mean_smooth_re_ind)),
      phi_lambda_par = rep(0, length(dat$phi_fp_mean_smooth_re))
    )
    ...
    if(use_phi_mean_smooth_re == 0) 
    {
      map$phi_lambda_par = factor(rep(NA,length(par$phi_lambda_par)))
      map$phi_mean_smooth_re = factor(rep(NA, length(par$phi_mean_smooth_re)))
    }
    ...
    if(dat$use_phi_mean_smooth_re == 1) random = c(random, "phi_mean_smooth_re")
  ...
  if(fit)
  {
    mod = MakeADFun(temp$dat, temp$par, DLL="betabin_gamm_nsmooth", map = temp$map, random= temp$random, silent = TRUE)
